...
typedef struct {
    ...
    XXts_aes_uio_map maps[ MAX_UIO_MAPS ];
} XXts_aes_uio_info;

/***************** Variable Definitions **************************************/
static XXts_aes_uio_info uio_info;
...
static int uio_info_read_name(XXts_aes_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XXts_aes_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XXts_aes_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XXts_aes_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XXts_aes_Initialize(XXts_aes *InstancePtr, const char* InstanceName) {
	XXts_aes_uio_info *InfoPtr = &uio_info;
