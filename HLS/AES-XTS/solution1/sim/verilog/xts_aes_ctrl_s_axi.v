...
#(parameter
    ...
    input  wire                          ap_ready,
    input  wire                          ap_idle,
    output wire [31:0]                   key_V,
    output wire [31:0]                   tweak_V,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_key_V = 'b0;
    reg  [31:0]                   int_tweak_V = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_TWEAK_V_DATA_0: begin
                    rdata <= int_tweak_V[31:0];
                end
                ADDR_TEXT_V_DATA_0: begin
                    rdata <= int_text_V[31:0];
                end
                ADDR_MODE_V_DATA_0: begin
                    rdata <= int_mode_V[15:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TWEAK_V_DATA_0)
            int_tweak_V[31:0] <= (WDATA[31:0] & wmask) | (int_tweak_V[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TEXT_V_DATA_0)
            int_text_V[31:0] <= (WDATA[31:0] & wmask) | (int_text_V[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MODE_V_DATA_0)
            int_mode_V[15:0] <= (WDATA[31:0] & wmask) | (int_mode_V[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_data_ret_V[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DATA_RET_V_DATA_0)
            int_data_ret_V[31:0] <= (WDATA[31:0] & wmask) | (int_data_ret_V[31:0] & ~wmask);
    end
end
